function [f1,R1,t1] = pnpfmy_orig(pts, Uc, tarPtNum, isFast, pnpOpts)        
%      pnpOpts.errThr = 2.5;
...
%         tic
        if (N == 1)
            [ Rest, test, fest, retVal] = pnp1_orig(C, V, D, tarPtNum, Cf, fans, pts, isFast);        
        else
            [ Rest, test, fest, retVal] = pnpNd1204Multi_orig( C, V, D, tarPtNum, Cf, fans, N, stopN, pts, isFast, pnpOpts.haveMex);        
        end
        if (doTimeMes)
            s5 = toc(s1)
        end        
...
function currentSolution = collectResult(retVal, Rest, test, fest, Uc, pts, currentSolution, isFast, pnpOpts)
    if (retVal > 0)        
        noDist = 0;
        [R1, t1, f1, k1est, avgerr] = refine_pos_dist_orig(Rest, test, fest, 0, Uc, pts, 0, ...
