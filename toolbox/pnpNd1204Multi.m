function [ Rest, test, fest, retVal, XN, avgerr] = pnpNd1204( C, V, D, tarPtNum, testvec, fans, N, stopN, pts, isFast, haveMex)    
    doTimeMes = 0;
...
%         if (1)
            ...
            info1 = ones(1,2)*norm(refine_f_res(fGuess, S)-R);
        end
        if (ret == -1)            
            poptf = fGuess;
        end
    ...
    [R_res, t_res, f, candsBest] = vote(cands, BP, C, resCosts, tarPtNum, pts);
    ...
    if (f < 0 && N<stopN)
        ...
        [candsBest, fest, avgerr] = optim_beta(candsBest, N, fest, L, R);
        XN = formBPAns(BP, candsBest', fest, tarPtNum);
